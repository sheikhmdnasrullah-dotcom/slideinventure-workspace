...
type Tool = {
  id: string; // matches the dashboard ?tab= value ("" for home)
  title: string;
  color: string;
};
...
function dashboardBase(): string {
  ...
  const raw = (cfg.get<string>("dashboardUrl") || "http://localhost:3000/ai-venture").trim();
  return raw.replace(/\/+$/, "");
}
...
function openTool(tool: Tool): void {
  const panel = vscode.window.createWebviewPanel(
    `toolbox.${tool.id || "home"}`,
    `AI Venture · ${tool.title}`,
...
class LauncherViewProvider implements vscode.WebviewViewProvider {
  ...
  resolveWebviewView(webviewView: vscode.WebviewView): void {
    ...
    webviewView.webview.onDidReceiveMessage((msg: { tool?: string }) => {
...
export function activate(context: vscode.ExtensionContext): void {
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider(
      "toolbox.launcher",
      new LauncherViewProvider(context.extensionUri)
    )
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("toolbox.openResearch", () => openById("research")),
    vscode.commands.registerCommand("toolbox.openPlayground", () => openById("playground")),
    vscode.commands.registerCommand("toolbox.openFiles", () => openById("files")),
    vscode.commands.registerCommand("toolbox.openUsefulLinks", () => openById("useful-links")),
    vscode.commands.registerCommand("toolbox.openQuery", () => openById("query")),
    vscode.commands.registerCommand("toolbox.openBrainstorm", () => openById("brainstorm")),
    vscode.commands.registerCommand("toolbox.openNotepad", () => openById("notepad")),
    vscode.commands.registerCommand("toolbox.openConnected", () => openById("connected")),
    vscode.commands.registerCommand("toolbox.openAgents", () => openById("agents")),
    vscode.commands.registerCommand("toolbox.openActivity", () => openById("activity"))
  );
}
