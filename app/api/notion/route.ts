...
export async function GET(request: NextRequest) {
  ...
  let q = supabase.from("notion_activity").select("*", { count: "exact" });

  if (query.data.eventType) q = q.eq("event_type", query.data.eventType);
  if (query.data.pageId) q = q.eq("page_id", query.data.pageId);
...
export async function POST(request: NextRequest) {
  ...
  const { error } = await supabase.from("notion_activity").insert({
    id,
    ...validated.data,
    created_at: now,
  });
