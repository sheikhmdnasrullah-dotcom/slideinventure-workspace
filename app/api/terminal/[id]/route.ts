...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await supabase.from("terminal_commands").select("*").eq("id", id).single();

  if (error || !data) return ApiError.notFound("COMMAND_NOT_FOUND", "Command not found").toResponse();
