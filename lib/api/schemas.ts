...
export const SecretVaultEntrySchema = z.object({
  ...
  username: z.string().optional().nullable(),
  secretType: z.enum(["password", "api_key", "secret", "private_key", "public_key", "role_key"]).default("secret"),
  url: z.preprocess((v) => (v === "" || v === null ? null : v), z.string().url().nullable().optional()),
  notes: z.string().optional().nullable(),
  tags: z.array(z.string()).default([]),
  encryptedValue: z.string().min(1),
  iv: z.string().min(1),
  keyVersion: z.number().int().positive().default(1),
  expiresAt: z.preprocess((v) => (v === "" || v === null ? null : v), z.string().datetime().nullable().optional()),
  createdBy: z.string().email().optional(),
  createdAt: z.string().datetime(),
  updatedAt: z.string().datetime(),
});
