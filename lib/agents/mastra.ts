...
import { listComposioConnections, getComposioSessionTools } from "@/lib/integrations/composio";
...
export async function runMastraAgent(opts: {
  ...
  // Real, per-action tools (e.g. GMAIL_SEND_EMAIL with a proper typed
  // schema) from Composio's session/Tool Router API, scoped to whatever
  // apps are actually connected. Empty object when nothing is connected —
  // spreads in as a safe no-op.
  const composioTools = await getComposioSessionTools().catch(() => ({}));
